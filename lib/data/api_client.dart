...
class ApiClient extends GetxService {
        ...
        for(MultipartDocument file in otherFile){
          if(kIsWeb) {
            request.files.add(http.MultipartFile.fromBytes(
              'other_documents[]', file.file!.bytes!,
              filename: file.file!.name,
            ));
          } else {
            io.File aaa = io.File(file.file!.path!);
  ...
  Future<Response> postMultipartDataConversation(
    ...
    if(otherFile != null) {
      if(kIsWeb) {
        request.files.add(http.MultipartFile.fromBytes('files[${multipartBody!.length}]', otherFile.bytes!, filename: basename(otherFile.name)));
      } else {
  ...
  Future<Response> postMultipartMergeWithImageAndDocument(
    ...
    if(documents != null) {
      for(MultipartDocument document in documents){
        if(kIsWeb) {
          request.files.add(http.MultipartFile.fromBytes('files[$i]', document.file!.bytes!, filename: basename(document.file!.name)));
        } else {
